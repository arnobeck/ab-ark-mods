<script>
	...
	let modList = "";
	$: modList = modListDetailed.map((m) => m.id).join(",");
</script>

{#if modList == "-999"}
	<div class="copypaste">
		<a href="/modpack/">Edit</a>
	</div>
{:else if modList}
	<div class="copypaste">
		<a href="/modpack/{modList}">Edit</a>
	</div>
{/if}
...
<style>
	...
	.copypaste > .icon:hover:after,
	.copypaste > .icon:hover:before {
		opacity: 1;
